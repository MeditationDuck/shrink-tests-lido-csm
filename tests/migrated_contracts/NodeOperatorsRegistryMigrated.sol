...
import {AragonApp} from "./aragon/os/apps/AragonApp.sol";
import {SafeMath} from "./aragon/os/lib/math/SafeMath.sol";
import {UnstructuredStorage} from "./aragon/os/common/UnstructuredStorage.sol";

import {Math256} from "../../core/contracts/common/lib/Math256.sol";
import {MinFirstAllocationStrategy} from "../../core/contracts/common/lib/MinFirstAllocationStrategy.sol";
import {ILidoLocator} from "../../core/contracts/common/interfaces/ILidoLocator.sol";
import {IBurner} from "../../core/contracts/common/interfaces/IBurner.sol";
import {SigningKeys} from "./utils_and_lib/SigningKeys.sol";
import {Packed64x4} from "./utils_and_lib/Packed64x4.sol";
import {Versioned} from "./utils_and_lib/Versioned.sol";
...
contract NodeOperatorsRegistryMigrated is AragonApp, Versioned {
    ...
    function decreaseVettedSigningKeysCount(
        ...
        for (uint256 i; i < nodeOperatorsCount;) {
            ...
            console.log("nodeOperatorId", nodeOperatorId);
            console.log("vettedKeysCount", vettedKeysCount);
    ...
    function addSigningKeysOperatorBH(uint256 _nodeOperatorId, uint256 _keysCount, bytes memory _publicKeys, bytes memory _signatures)
        external
    {
        _addSigningKeys(_nodeOperatorId, _keysCount, _publicKeys, _signatures);
    }
    ...
    function clearNodeOperatorPenalty(uint256 _nodeOperatorId) external returns (bool) {
        Packed64x4.Packed memory stuckPenaltyStats = _loadOperatorStuckPenaltyStats(_nodeOperatorId);
        console.log("isOperatorPenalized", _isOperatorPenalized(stuckPenaltyStats));
        console.log("stuckPenaltyStats.get(STUCK_PENALTY_END_TIMESTAMP_OFFSET)", stuckPenaltyStats.get(STUCK_PENALTY_END_TIMESTAMP_OFFSET));
        require(
    ...
    function getNodeOperatorIds(uint256 _offset, uint256 _limit)
        ...
        uint256 nodeOperatorsCount = getNodeOperatorsCount();
        if (_offset >= nodeOperatorsCount || _limit == 0) return nodeOperatorIds;
    ...
    function _authP(bytes32 _role, uint256[] memory _params) internal view {
